...
import { Package, ShoppingBag, Trophy, ArrowLeft, Clock, CreditCard, ChevronRight } from 'lucide-react';
    ...
    const getStatusBadge = (status: TransactionStatus) => {
        switch (status) {
            case TRANSACTION_STATUS.PENDING_PAYMENT:
                return <Badge className="bg-amber-50 text-amber-700 border-amber-100 hover:bg-amber-50">{t('transaction.status.pending_payment')}</Badge>;
        ...
        >
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex border-b border-[#f0f2f5] overflow-x-auto no-scrollbar">
                    ...
                    >
                        {t('marketplace.filters.all_categories').split(' ')[1] || 'All'}
                    </button>
                    <button
                ...
                <div className="flex flex-col gap-3">
                    {filteredItems.length > 0 ? (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center justify-between mt-1">
                                            <div className="flex items-center gap-3 text-sm text-[#5f6c84]">
                                                ...
                                                <span>•</span>
                                                <span>{t('listing.sidebar.seller')}: <span className="text-[#2b4b8f] font-medium">{item.seller.name}</span></span>
                                            </div>
                                            <Link
                                                href={item.transaction_id ? `/transactions/${item.transaction_id}` : `/listings/${item.id}`}
                        ...
                        <Card className="rounded-2xl border-dashed border-2 border-[#f0f2f5] p-12 text-center text-[#5f6c84] bg-white">
                            ...
                            <p className="font-medium">{t('dashboard.won_items.no_items')}</p>
                            <Link href="/marketplace" className="inline-flex items-center gap-2 mt-4 text-[#0d9488] font-bold hover:underline bg-[#f0fdfa] px-6 py-2 rounded-full">
